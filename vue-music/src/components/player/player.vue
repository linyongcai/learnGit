        ...
        >
          <div class="middle-l" ref="middleL">
            ...
            <div class="playing-lyric-wrapper">
              <div class="playing-lyric" :class="{'current': index===0}" v-for="(line,index) in playingLyric">{{line}}
              </div>
            </div>
          </div>
    ...
    methods: {
      ...
      getLyric() {
        this.currentSong.getLyric().then((lyric) => {
          ...
          this.lyricArr = this.currentLyric.lines
      ...
      lyricHandler({lineNum, txt}) {
        ...
        if (!this.playingLyric.length) {
          this.playingLyric.push(txt)
        }
        if (lineNum < this.lyricArr.length - 1) {
          this.playingLyric.push(this.lyricArr[lineNum + 1].txt)
        }
        if (this.playingLyric.length > 2 || lineNum === this.lyricArr.length - 1) {
          this.playingLyric.shift()
        }
        if (lineNum > 5) {
      ...
      middleTouchMove(e) {
        ...
        let detailX = moveX - this.touch.pageX
        let detailY = moveY - this.touch.pageY
        ...
        this.$refs.middleL.style.opacity = this.touch.opacity
